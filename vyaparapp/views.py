...
def create_paymentout(request):
    if request.method == 'POST':
        ...
        pbill = PaymentOut(
            ...
            company=cmp,
            party=part,
            billno=part.id,  # Use party id as billno
            billdate=request.POST.get('billdate'),
            pay_method=request.POST.get("method"),
            cheque_no=request.POST.get("cheque_id"),
            upi_no=request.POST.get("upi_id"),
        ...
        # Handle the case where no PurchaseBill is found for the party
        return render(request, 'error_page.html', {'error_message': 'No matching PurchaseBill found for the given party'})
